...
<div class="main">
  <div class="main-inner">
	 ...
	 <div class="row">
		...
		if(($_GET["delete"]) == "berhasil"){
		 echo "<div class='alert'>
							<span style=''> File berhasil dihapus. </span>
						  </div>";}}
			?>
            <!-- /widget-header -->
        ...
        <tr>
            <td><?php if(isset($_GET["edit"])){
				...
				$id_file= $_GET['edit'];
				if($id_file==$fetch['id_file']){
				$query1 ="select * from upload where id_file='$id_file'";
				$data1 = mysqli_query($con,$query1);
				$fetch1 = mysqli_fetch_array($data1);
				$nama = $fetch1['nama_file'];
				$id = $fetch1['id_file'];
				echo "<form method='POST' action='update-file.php?id_file=$id'>
						<input size='10' type='text' class='form' name='nama_file' id='nama_file' value='$nama'>";
						
						}
				else echo $fetch['nama_file']; 
				}
				else echo $fetch['nama_file']; 
				?>
				</td>
			...
			<td><?php if(isset($_GET["edit"])){
				...
				if($id_file==$fetch['id_file']){
				$query2 ="select * from upload where id_file='$id_file'";
				$data2 = mysqli_query($con,$query2);
				$fetch2 = mysqli_fetch_array($data2);
				$desk = $fetch2['deskripsi'];
				$id = $fetch2['id_file'];
				echo "
						<input size='25' type='text' class='form' name='deskripsi' id='deskripsi' value='$desk'>
						<input type='submit' value='Ok' class='btn btn-primary'></form>";
						}
				else echo $fetch['deskripsi'];
				}
				 echo $fetch['deskripsi']; ?></td>
			<td>
			<button button class="btn"><a href="halaman-upload.php?edit=<?php echo $fetch['id_file'];?>"> <font color="black">Edit</font></a></button>
		    <button button class="btn btn-primary"><a href="<?php echo $fetch['file'];?>"> <font color="white">Lihat</font></a></button>
	...
	{echo"<div class='alert'>
	<span style=''> Data belum ada yang di upload </span>
	</div>";}
